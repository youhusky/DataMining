...
def main():
	...
	standard_value = standard_class(raw_data)
	raw_merge, value = pair(raw_data)
...
def standard_class(raw_data):
	var = [raw_data[classname][4] for classname in range(len(raw_data))]
	var = list(set(var))
	object_standard_class = []
	for each_classname in range(len(var)):
		temp = []
		for object_index in range(len(raw_data)):
			if raw_data[object_index][4] == var[each_classname]:
...
def pair(data):
	...
	for object_index_i in range(len(data)):
		object_value.append([object_index_i, data[object_index_i][:4]])
		for object_index_j in range(object_index_i + 1, len(data)):
			merged_distance.append([euclidean_distance(data[object_index_i][:4],
			                                 data[object_index_j][:4]),
